...
export function PageHero({ eyebrow, title, accent, sub, image, children, contrastVeil }: {
  eyebrow: string;
  title: string;
  accent?: string;
  sub?: string;
  image?: string;
  children?: ReactNode;
  /** Adds a localized dark scrim behind the text column for pages whose hero
   *  imagery has bright frames. Keeps the image itself at full brightness. */
  contrastVeil?: boolean;
}) {
  return (
    <section className="relative isolate overflow-hidden pt-[calc(var(--hdr-h,64px)+3.5rem)] pb-16 md:pt-[calc(var(--hdr-h,80px)+4.5rem)] md:pb-24">
      ...
      {contrastVeil && (
        <>
          <div
            aria-hidden="true"
            className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(120%_90%_at_18%_45%,color-mix(in_oklab,var(--onyx)_78%,transparent),transparent_70%)]"
          />
          <div
            aria-hidden="true"
            className="pointer-events-none absolute inset-x-0 bottom-0 -z-10 h-2/3 bg-[linear-gradient(180deg,transparent,color-mix(in_oklab,var(--onyx)_72%,transparent))]"
          />
        </>
      )}
